    ...
    if annotFile.count('refseq') == 1:
        ...
        for gene in geneList:
            if gene in refseqDict == False:
                continue
            Dict[gene]={}
            Dict[gene]['sense'] = refseqTable[refseqDict[gene][0]][3]
            Dict[gene]['chr'] = refseqTable[refseqDict[gene][0]][2]
            Dict[gene]['start'] = getTSSs([gene],refseqTable,refseqDict)
            if Dict[gene]['sense'] == '+':
                Dict[gene]['end'] =[int(refseqTable[refseqDict[gene][0]][5])]
            else:
                Dict[gene]['end'] = [int(refseqTable[refseqDict[gene][0]][4])]
            Dict[gene]['name'] = refseqTable[refseqDict[gene][0]][12]
    return Dict
...
class LocusCollection:
    ...
    def hasLocus(self,locus):
        return locus in self.__loci.keys()
    def remove(self,old):
    ...
    def getChrList(self):
        ...
        # them non-redundant.
        tempKeys = dict()
        for k in self.__chrToCoordToLoci: tempKeys[k[:-1]] = None
